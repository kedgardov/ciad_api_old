...
try {
    ...
    $id_objetivo = isset($input['id_objetivo']) ? filter_var($input['id_objetivo'], FILTER_VALIDATE_INT) : null;

    if ($id_curso === false || $id_objetivo === false || $id_curso <= 0 || $id_objetivo <= 0) {
    ...
    if (isset($_SESSION['username'])) {
        ...
        if (userHasPermissionsInCurso($username, $id_curso, $conn)) {
            $sql = "
                ...
                WHERE id_objetivo = ? AND id_curso = ?
            ";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param('ii', $id_objetivo, $id_curso);
            $stmt->execute();

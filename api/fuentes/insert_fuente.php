...
try {
    ...
    if ($id_curso === 0 || $fuente === null || $id_tipo_fuente === 0) {
    ...
    if (isset($_SESSION['username'])) {
        ...
        if ($hasPermissions) {
            ...
            $sql = "
            INSERT INTO fuentes
            (id_curso, titulo, id_tipo_fuente, cita)
            VALUES (?, ?, ?, ?)
            ";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param('isis', $id_curso, $titulo, $id_tipo_fuente, $cita);
            $stmt->execute();

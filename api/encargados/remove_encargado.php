...
try {
    ...
    $id_rol_curso = isset($input['id_rol_curso']) ? filter_var($input['id_rol_curso'], FILTER_VALIDATE_INT) : 0;
    $id_curso = isset($input['id_curso']) ? filter_var($input['id_curso'], FILTER_VALIDATE_INT) : 0;
    // Validate required parameters
    if ($id_rol_curso === false || $id_curso === false) {
    ...
    if (isset($_SESSION['username'])) {
        ...
        if ($hasPermissions) {
            ...
            $sql = "DELETE FROM roles_cursos WHERE id_rol_curso = ?";
            $stmt = $conn->prepare($sql);
            if (!$stmt) {
                throw new Exception('Failed to prepare statement: ' . $conn->error);
            }

            $stmt->bind_param('i', $id_rol_curso);
            $stmt->execute();
            ...
            throw new Exception('User is not allowed to delete this encargado');
        }
    } else {

...
try {
    ...
    $id_criterio = isset($input['id_criterio']) ? filter_var($input['id_criterio'], FILTER_VALIDATE_INT) : 0;
    $id_curso = isset($input['id_curso']) ? filter_var($input['id_curso'], FILTER_VALIDATE_INT) : 0;
    // Validate required parameters
    if ($id_criterio === false || $id_curso === false) {
    ...
    if (isset($_SESSION['username'])) {
        ...
        // Verify user permissions (assuming userHasPermissionsInCriterio is defined similarly)
        ...
        if ($hasPermissions) {
            ...
            $sql = "DELETE FROM criterios WHERE id_criterio = ?";
            ...
            throw new Exception('User is not allowed to delete this criterio');
        }
    } else {

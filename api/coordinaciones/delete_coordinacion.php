...
try {
    ...
    $id_coordinacion_curso = isset($input['id_coordinacion_curso']) ? filter_var($input['id_coordinacion_curso'], FILTER_VALIDATE_INT) : null;

    if ($id_curso === false || $id_coordinacion_curso === false || $id_curso <= 0 || $id_coordinacion_curso <= 0) {
    ...
    if (isset($_SESSION['username'])) {
        ...
        if (userHasPermissionsInCurso($username, $id_curso, $conn)) {
            $sql = "
                DELETE FROM coordinaciones_cursos
                WHERE id_coordinacion_curso = ? AND id_curso = ?
            ";

            $stmt = $conn->prepare($sql);
            $stmt->bind_param('ii', $id_coordinacion_curso, $id_curso);
            $stmt->execute();

...
try {
    ...
    $lgac = isset($input['lgac']) ? $input['lgac'] : null;

    if (!$lgac) {
        throw new Exception('Invalid parameters');
    }

    $id_lgac = isset($lgac['id_lgac']) ? filter_var($lgac['id_lgac'], FILTER_VALIDATE_INT) : null;
    $id_programa = isset($lgac['id_programa']) ? filter_var($lgac['id_programa'], FILTER_VALIDATE_INT) : null;
    $id_modalidad = isset($lgac['id_modalidad']) ? filter_var($lgac['id_modalidad'], FILTER_VALIDATE_INT) : null;

    // Validate required parameters
    if ($id_curso === false || $id_lgac === false || $id_programa === false || $id_modalidad === false) {
    ...
    if (isset($_SESSION['username'])) {
        ...
        if ($hasPermissions) {
            $sql = "
                INSERT INTO lgacs_cursos (id_curso, id_lgac, id_programa, id_modalidad)
                VALUES (?, ?, ?, ?)
            ";
            ...
            $stmt->bind_param('iiii', $id_curso, $id_lgac, $id_programa, $id_modalidad);
            $stmt->execute();
            ...
            $response = [
                'success' => true,
                'id_lgac_curso' => $stmt->insert_id
            ];

            echo json_encode($response);
        } else {
            throw new Exception('User is not allowed to insert this LGAC');
        }
    } else {

...
try {
    ...
    $id_lgac_curso = isset($input['id_lgac_curso']) ? filter_var($input['id_lgac_curso'], FILTER_VALIDATE_INT) : 0;
    $id_curso = isset($input['id_curso']) ? filter_var($input['id_curso'], FILTER_VALIDATE_INT) : 0;
    // Validate required parameters
    if ($id_lgac_curso === false || $id_curso === false) {
    ...
    if (isset($_SESSION['username'])) {
        ...
        if ($hasPermissions) {
            ...
            $sql = "DELETE FROM lgacs_cursos WHERE id_lgac_curso = ?";
            $stmt = $conn->prepare($sql);
            if (!$stmt) {
                throw new Exception('Failed to prepare statement: ' . $conn->error);
            }

            $stmt->bind_param('i', $id_lgac_curso);
            $stmt->execute();
            ...
            throw new Exception('User is not allowed to delete this LGAC');
        }
    } else {

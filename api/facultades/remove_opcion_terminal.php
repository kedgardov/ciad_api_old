...
try {
    ...
    $id_opcion_terminal_curso = isset($input['id_opcion_terminal_curso']) ? filter_var($input['id_opcion_terminal_curso'], FILTER_VALIDATE_INT) : 0;
    $id_curso = isset($input['id_curso']) ? filter_var($input['id_curso'], FILTER_VALIDATE_INT) : 0;
    // Validate required parameters
    if ($id_opcion_terminal_curso === false || $id_curso === false) {
    ...
    if (isset($_SESSION['username'])) {
        ...
        if ($hasPermissions) {
            ...
            $sql = "DELETE FROM opciones_terminales_cursos WHERE id_opcion_terminal_curso = ?";
            $stmt = $conn->prepare($sql);
            if (!$stmt) {
                throw new Exception('Failed to prepare statement: ' . $conn->error);
            }

            $stmt->bind_param('i', $id_opcion_terminal_curso);
            $stmt->execute();
            ...
            throw new Exception('User is not allowed to delete this Opcion Terminal');
        }
    } else {
